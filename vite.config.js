...
function renderSeoFallback(route, title, description) {
  ...
  return `<div id="root"><main><h1>${escapeHtml(details.heading)}</h1><p>${escapeHtml(details.summary)}</p><h2>${escapeHtml(title.replace(/ \| Kabin$/, ""))}</h2><p>${escapeHtml(description)}</p>${list}</main></div>`;
}
